...
AAD B2B enables you to securely collaborate with external parners.
...
External Identities > User Flow > App Registration/AAD B2C tenant
...
- Use AAD Connect or AD  Connect cloud sync for hybrid identity sync

On-premises Identities (ADDS) > AAD Connect > AAD (internal users, on-premise users, guest users(B2B))
...
A typical Azure hierarchy has 4 levels (RM scopes):
...
Target (resource to grant permission to):
...
Q02: An organization has an existing Azure AD tenan. They plan to deploy multiple Azure Cosmos DB databases and will use the SQL API. You are asked to recommend a solution that provides AAD user accounts with read access to the Cosmos DB databases. 
...
- You need to host enterprise SQL Server applications in the cloud and have full control over the server OS = SQL Server on Azure VMs
...
**Ingest** (IoT devices, Logs, Files, Customer data, Financial transactions, weather data, business apps) > Event Hubs/Azure Blob Storage/IoT Hub > **Analyze**(Continous Intelligence/Real-time analytics, Stream Analytics) using Reference Data (SQL DB, Blob store), Real-time scoring (Azure ML service) > **Deliver** Alerts and actions (Event Hubs, Service Bus, Azure Functions etc.), Dynamic Dashboarding (Power BI), Data Warehousing (Azure Synapse Analytics), Storage/Archival (SQL DB, Azure Data Lake Gen1 & Gen2, Cosmos DB, Blob storage, and so on)
...
SQL Virtual machines - Best for migrations and applications requiring OS level access
Managed instances - Best for most lift-and-shift migrations to the cloud
Databases - Best for modern cloud applications
