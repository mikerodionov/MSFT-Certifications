...
- [Caching types and best practices](https://docs.microsoft.com/en-us/azure/architecture/best-practices/caching)
    - **Private caching** - one data persistent storage, aplication instances retrieve data and then store it in its in-memory cache (e.g. SQL server and multiple insances of app); as each instance has its own in-memory cache this approach is called private caching
    - **Shared caching** - one data persistance storage with shared service cache in front of it used by multiple application instances
...
- CDN - efficiently delivers web content to users via caching; improves performance through content delivery through servers which are physically closer to consumers (point of presence locations)
...
- When to use Azure Cache for Redis
    - When you need managed service copatible with Redis
...
- Azure cache for Redis tiers
    ...
    - Enterprise Flash
        - based on commercial implementation from Redis Labs
        - extens in-memory storage over to a data storage for cost efficiency
...
- Scaling Azure Cache for Redis instance
...
- [Selecting Redis (pricing) tiers](https://azure.microsoft.com/en-us/pricing/details/cache/#pricing)
    ...
    - Memory size 50 GB = Basic/Standard is OK
    - Max number of client connections 10,000 = Basic/Standard is OK
